...
P-value is greater than 0.05, so all looks good. We will run our ANOVA on log_secchi_m.
...
***Challenge 8:*** *Now, finish creating the `season` column using `case_when` for "Autumn", "Winter" and "Spring". You will need to add `TRUE ~ season` as the last argument so that the values we set for Summer in the previous chunk of code remain.*
